"""
CyberBackgroundChecks.com scraper implementation
"""
...
class CyberBackgroundChecksScraper(BaseScraper):
    """Scraper for CyberBackgroundChecks.com"""
    ...
    def _build_search_url(
        ...
        # CyberBackgroundChecks uses path: /people/Firstname/Lastname or /people/Firstname/Lastname/State
    ...
    async def search(
        ...
        try:
            ...
            logger.info(f"CyberBackgroundChecks: Searching at {search_url}")
            ...
            if "just a moment" in page_title or "cloudflare" in page_source:
                logger.warning("CyberBackgroundChecks: Cloudflare protection detected, waiting...")
                await self.random_delay(5, 10)

            if "blocked" in page_title or "access denied" in page_source:
                logger.error("CyberBackgroundChecks: Access blocked")
                return ScraperResult(
                    source=self.source_name,
                    success=False,
            ...
            if not data or not data.get("results"):
                # Save debug info
                try:
                    self.driver.save_screenshot("cyberbackgroundchecks_debug.png")
                    with open("cyberbackgroundchecks_page.html", "w", encoding="utf-8") as f:
                        f.write(self.driver.page_source)
                    logger.info("DEBUG: Saved CyberBackgroundChecks screenshot and HTML")
            ...
            logger.error(f"CyberBackgroundChecks search failed: {e}", exc_info=True)
            return ScraperResult(
                source=self.source_name,
                success=False,
                error=str(e)
        ...
        try:
            ...
            try:
                ...
                if age_match:
                    ...
                    # Try finding standalone numbers that could be age (20-100)
            ...
            for sel in location_selectors:
                try:
                    ...
                    for elem in elems:
                        text = elem.text.strip()
                        # Check if it looks like an address (contains state abbreviation or zip)
